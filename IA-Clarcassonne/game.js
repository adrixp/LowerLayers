    ...
    ColocarSeguidor:function(id_partida, id_jugador, coordenada, seguidor){
	  ...
	  if (Tablero.totalFichas == 0){
		...
		for (i=0; i< Tablero.listaJugadores.length; i++){
	  	...
	  	Partidas.update({_id:id_partida},{$set:{terminada: true, puntuacion: puntuacion}});
      console.log("actualizando partida a terminada y con puntuaciones")
		matchMulti(id_partida);
    console.log("hemos llamado al metedo finis de la pl")
	  }
